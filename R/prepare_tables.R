...
prepare_cohort_boundaries <- function(x) {
  x %>%
    summarise(
      ...
      first_visit_dttm = as.POSIXct(min(visit_start_datetime, na.rm = TRUE)),
      last_visit_date = max(visit_start_date, na.rm = TRUE),
      last_visit_dttm = as.POSIXct(max(visit_start_date, na.rm = TRUE)),
      first_dc_date = min(visit_end_date, na.rm = TRUE),
      first_dc_dttm = as.POSIXct(min(visit_end_datetime, na.rm = TRUE)),
      last_dc_date = max(visit_end_date, na.rm = TRUE),
      last_dc_dttm = as.POSIXct(max(visit_end_datetime, na.rm = TRUE))
    ) %>%
